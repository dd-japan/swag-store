...
func getProduct(ctx context.Context, id string) {

	// URLに商品IDを組み込む
	url := "http://10.244.0.201:3551/product/" + id

	// 新しいHTTPリクエストをコンテキストと共に作成
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		log.Fatalf("Error occurred during creating request. Error: %s", err.Error())
	}
	...
	if err != nil {
		log.Fatalf("Error occurred during making request. Error: %s", err.Error())
	}
	...
	log.Println("Product Details:", string(body))
}
